...
-- 定义颜色
display.COLOR_LIGHTYELLOW = cc.c3b(231, 217, 185)
display.COLOR_DARKYELLOW = cc.c3b(66, 40, 32)
display.COLOR_YELLOW = cc.c3b(255, 255, 0)
display.COLOR_BLUE = cc.c3b(0, 153, 204)
display.COLOR_GREEN = cc.c3b(11, 241, 6)
display.COLOR_BLACK = cc.c3b(0, 0, 0)
display.COLOR_ORANGE = cc.c3b(255, 144, 0)
display.COLOR_ORANGERED = cc.c3b(248, 82, 0)
display.COLOR_WHITE = cc.c3b(255, 255, 255)
display.COLOR_LIGHTGRAY = cc.c3b(169, 169, 169)
display.COLOR_LIGHTGREEN = cc.c3b(50, 205, 50)
display.COLOR_DARKBLUE = cc.c3b(0, 139, 139)
display.COLOR_FONT = cc.c3b(36, 36, 36)	
display.COLOR_FONTGREEN = cc.c3b(2, 70, 30)	-- 绿色按钮上 cc.c3b(2, 99, 46)
display.COLOR_BUTTON_STROKE = cc.c3b(36, 36, 36)	-- 所有按钮上描边 #242424
display.COLOR_SHADOW = cc.c3b(0, 0, 0)
display.COLOR_BROWN = cc.c3b(111, 85, 71)
display.COLOR_DARKBROWN = cc.c3b(66, 45, 23)
display.COLOR_BORDER_BROWN = cc.c3b(49, 22, 0)
display.COLOR_BROWNSTROKE = cc.c3b(77, 42, 29)
display.COLOR_ORANGE = cc.c3b(254, 201, 16) --橙色
display.COLOR_LIGHTBLUE = cc.c3b(0, 255, 216) --水蓝色
...
function switchScene(name, params, callback, transitionParams)
	...
	local scene = require(string.format("app.scenes.%s", name))
...
function uihelper.shake(args, target)
	...
	local actions = {}
	actions[#actions + 1] = cc.MoveBy:create(time, ccp(x, y))
	for count = 1, shakeCount do
		actions[#actions + 1] = cc.MoveBy:create(time * 2, ccp(0 - x * 2, 0 - y * 2))
		actions[#actions + 1] = cc.MoveBy:create(time * 2, ccp(x * 2, y * 2))
	end
	actions[#actions + 1] = cc.MoveBy:create(time, ccp(-x, -y))
...
function uihelper.createStroke(params)
	...
	local strokeColor = params.strokeColor or cc.c3b(0, 0, 0)
...
function uihelper.createLabel(params)
	...
	for index = #texts, 1, -1 do
		...
		local height = line * lineHeight
		totalLine = totalLine + line
		label:setDimensions(CCSizeMake(totalWidth, height))
		label:setHorizontalAlignment(ui.TEXT_ALIGN_LEFT)
		label:anch(0, 0):pos(0, yPos):addTo(layer)
		yPos = yPos + height
	end  
	layer:size(totalWidth, yPos)      
...
function uihelper.popupNode(node, scale)
	...
	local actions = transition.sequence({
		cc.ScaleTo:create(0.1, 1.2 * scale),
		cc.ScaleTo:create(0.2, 0.9 * scale),
...
function uihelper.shrinkNode(node, onComplete)
	local actions = transition.sequence({
		...
		cc.CallFunc:create(function() onComplete() end)
	})
...
function uihelper.hex2rgb(hex)
    hex = hex:gsub("#","")
    return cc.c3b(tonumber("0x"..hex:sub(1,2)), 
...
function pushLayerAction(node,isRun)
	if isRun then
		...
		node:runAction(transition.sequence({
			-- CCEaseOut:create(cc.ScaleTo:create(0.1, 1.3), 10),
			cc.ScaleTo:create(0.1, 1.2),
...
function showMaskLayer(params)
	...
	mask:runAction(transition.sequence({
		cc.DelayTime:create(params.delay or 3),
		cc.RemoveSelf:create(),
		}))
	return mask
end
	...
	for key, value in pairs(EquipAttEnum) do
		...
		if deltaValue ~= 0 then
			...
			text:runAction(transition.sequence({
				cc.CallFunc:create(function() uihelper.fadeTree({node = text, from = 255, to = 0, effectTime = 0}) end),
				cc.DelayTime:create(delayTime),
				-- CCSpawn:createWithTwoActions(CCFadeIn:create(0.1), cc.MoveBy:create(0.15, ccp(0, deltaY))),
				cc.CallFunc:create(function() uihelper.fadeTree({node = text, from = 0, to = 255, effectTime = fadeInTime}) end),
				CCSpawn:createWithTwoActions(cc.DelayTime:create(fadeInTime), cc.MoveBy:create(0.15, ccp(0, deltaY))),
				cc.DelayTime:create(0.65),
				-- CCSpawn:createWithTwoActions(CCFadeOut:create(0.15), cc.MoveBy:create(0.15, ccp(0, deltaY))),
				cc.CallFunc:create(function() uihelper.fadeTree({node = text, from = 255, to = 0, effectTime = fadeOutTime}) end),
				CCSpawn:createWithTwoActions(cc.DelayTime:create(fadeOutTime), cc.MoveBy:create(0.15, ccp(0, deltaY))),
				cc.RemoveSelf:create(),
				}))
...
local colorSchema = {
	...
	[2] = cc.c3b(0, 255, 0),  -- display.COLOR_GREEN
	[3] = cc.c3b(0, 150, 255),  -- display.COLOR_BLUE
	[4] = cc.c3b(225, 0, 225), -- 紫色
